" git 提交插件 by yangqing
" date@2017-12-29
" 使用说明：
" 直接调用Q_GitStatus函数即可,在vimrc中增加如下配置:
" `command! Qgit :call Q_GitStatus()`
" 配置完成后输入:Qgit
" ====================================
" 窗口中快捷键
" 按键o 加入提交列表
...
let s:brief_help = 1
let s:file_count = 0
if !exists('g:git_async')
    let g:git_async = 0
endif

if !exists('g:git_ignore')
    let g:git_ignore = 0
endif
...
function! s:Window_Display_Help()
    if s:brief_help
        call append(0, '" GIT 管理工具(1.0)  *查看帮助<F1>, 说明："o"选择，"x"取消，"s"提交')
        call append(1, '" =============文件列表==============')
    else
        " Add the extensive help
        call append(0, '" GIT 管理工具(1.0)')
        call append(1, '" =============功能列表==============')
        call append(2, '" o/<enter> : 将文件加入到待提交列表')
        call append(3, '" x : 将文件从待提交列表中取消')
        call append(4, '" t : 在新标签打开文件')
        call append(5, '" r : 回滚文件[git checkout]')
        call append(6, '" d : 对比文件与版本库中的差异[git diff]')
        call append(7, '" u : 更新所有文件[git update]***')
        call append(8, '" a : 其他')
        call append(9, '" s : 将待提交列表中的文件提交到git[git commit]')
...
function! Q_GitStatus(...)
    ...
    echo 'git status ...'
    let cmd = 'git st'
    let cmd = cmd.' | egrep -v "'.join(g:git_ignore, "|").'"'
    ...
    " 加入提交列表
    map <silent><buffer> <CR> :call Q_GitSet(line('.'), getline('.'))<CR>
    map <silent><buffer> o :call Q_GitSet(line('.'), getline('.'))<CR>
    " 删除提交列表
    map <silent><buffer> d :call Q_GitUnset(line('.'), getline('.'))<CR>
    map <silent><buffer> x :call Q_GitUnset(line('.'), getline('.'))<CR>
    map <silent><buffer> t :call Q_GitOpen(getline('.'))<CR>
    map <silent><buffer> r :call Q_GitRervert(getline('.'))<CR>
    map <silent><buffer> d :call Q_GitDiff(getline('.'))<CR>
    map <silent><buffer> u :call Q_GitUpdate()<CR>
    " 提交到git
    map <silent><buffer> s :call Q_GitCommit()<CR>
    echohl None
endfunction

function! Q_GitUpdate(...)
    let msg = input('确定要更新当前项目吗？ Y/n：')
    if msg == 'Y'
        call Q_GitExecute("git update ", 1)
        call Q_GitStatus()
    endif
endfunction


function! Q_GitRervert(...)
    ...
    if msg == 'Y'
        ...
        call Q_GitExecute("git revert ".item_file)
        call Q_GitStatus()
    endif
endfunction
...
function! Q_GitDiff(...)
    ...
    if item_type == 'M'
    let item_file = var_item[-1]
        call Q_GitExecute("git diff ".item_file, 1)
    else
...
function! Q_GitCommit()
    ...
    " 拼装需要新增的文件列表
    let git_cmd_add = "git add "
    if !empty(add_list)
        for item in add_list
            let git_cmd_add = git_cmd_add.' '.item
        endfor
        echohl WarningMsg | echo git_cmd_add | echohl None
    else
        let git_cmd_add = ""
    endif
    " 拼装需要提交的文件列表
    let git_cmd_commit = "git commit -m 'MESSAGE' "
    if !empty(update_list)
        for item in update_list
            let git_cmd_commit = git_cmd_commit.' '.item
        endfor
        echohl WarningMsg | echo git_cmd_commit | echohl None
    else
    ...
    if msg  == ""
        ...
        let git_cmd_commit = substitute(git_cmd_commit, "MESSAGE", msg, "")
        if git_cmd_add != ""
            call Q_GitExecute(git_cmd_add, 2)
        endif
        call Q_GitExecute(git_cmd_commit, 1)
         call Q_GitStatus()
        " echohl SpellLocal | echo '操作完成' | echohl None
    endif
endfunction

function! Q_GitExecute(...)
    if a:0 >1 && a:2 == 1
        execute "!".a:1
        return 1
    elseif a:0 >1 && a:2 == 2
